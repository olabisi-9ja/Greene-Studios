...
> **Why this file is in the Greene-Studios repo.** It isn't Greene Studios work. The
> planning session ran from a local `Comuta/` folder that has no git of its own — it sits
> inside `C:\Users\Abel ADIGUN`, which *is* a checkout of Greene-Studios. The plan existed
> only in a local `.claude/plans/` scratch file, which is not durable. This directory is a
> deliberate, namespaced parking spot so the plan survives. **Nothing here is imported by
> the Greene Studios site.** Move `docs/comuta/` into the Comuta monorepo (as `docs/` and
> `design/figma-manifest.json`) the moment that repo is cloned, and delete it from here.
...
Machine-readable and complete in **[`design-tokens.json`](./design-tokens.json)** — six M3
...
**[`figma-manifest.json`](./figma-manifest.json)** — re-verified 2026-09-01, all 402×874,
