...
export const ExperienceHero = () => {
  return (
    <section className="relative min-h-[100svh] w-full flex flex-col justify-between overflow-hidden bg-[var(--brand-bg)] text-[var(--brand-text)] transition-colors duration-1000 p-6 md:p-12 pt-28 md:pt-32 pb-6 md:pb-12 z-0">
      ...
      {/* Massive Center Text (pushes bottom content down) */}
      <motion.div 
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
        className="flex-1 flex flex-col items-start justify-center w-full pointer-events-none select-none overflow-visible my-12 relative z-10"
      >
        <h1 
          className="font-black leading-[0.75] tracking-tighter text-[var(--brand-text)] whitespace-nowrap pl-[2vw]"
          style={{ fontSize: "clamp(5rem, 25vw, 40rem)" }}
        >
          GREENE
        </h1>
        <div 
          className="font-bold tracking-widest text-[var(--brand-text)] uppercase opacity-80 pl-[10vw]"
      ...
      {/* Bottom Row */}
      <div className="flex flex-col md:flex-row justify-start md:justify-between items-start md:items-end w-full font-bold gap-6 md:gap-0 pointer-events-auto relative z-20">
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
